...
public class StrongPasswordValidator implements ConstraintValidator<StrongPassword, String> {
    ...
    public boolean isValid(String password, ConstraintValidatorContext context) {
        ...
        if (password.length() < 8) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("密码长度不能少于8位").addConstraintViolation();
            return false;
        }
        if (WEAK_PASSWORDS.contains(password.toLowerCase())) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("密码过于简单，请使用更复杂的密码").addConstraintViolation();
            return false;
        }
        int types = 0;
        ...
        if (types < 3) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("密码需包含大写字母、小写字母、数字、特殊字符中的至少3种").addConstraintViolation();
            return false;
        }
        return true;
    }
